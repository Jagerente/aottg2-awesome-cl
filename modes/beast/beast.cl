class Main
{
	...
	function OnFrame()
	{
		...
		{
			self._spawnLight.Position = Vector3.Lerp(self._spawnLight.Position, Vector3(0, 1000, 0), Time.FrameTime * 0.5);
		}
	}
...
cutscene Cutscene_1
{
	...
	coroutine Start()
	{
		...
		Map.FindMapObjectByName("TitansFloor").Active = false;
...
{
	_thunderFxOffset = Vector3.Up * 90.0 + Vector3(0.0, 0.0, -15.0);
	_spawnFxOffset = Vector3.Up * 90.0 + Vector3(0.0, 0.0, -15.0);
	_spawnFxSize = 5.0;
	...
	function Spawn(type, pos)
	{
		...
		{
			Game.SpawnEffect("ShifterThunder", fxPos + self._thunderFxOffset, Vector3.Zero, i * 1.5);
		}
